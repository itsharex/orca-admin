...
self.onmessage = async e => {
  ...
  const { chunkList, fileHash } = (await sliceFile(targetFile, baseChunkSize)) as { chunkList: ArrayBuffer[]; fileHash: string };
  // 将chunkList分批次发送,每次发送10条
  const batch = 10;
  for (let i = 0; i < chunkList.length; i += batch) {
    const chunkListBatch = chunkList.slice(i, i + batch);
    // 发送分片列表和文件hash
    self.postMessage({ chunkList: chunkListBatch, fileHash, length: chunkList.length });
  }
};
 ...
 */
async function sliceFile(targetFile: { size: number }, baseChunkSize = 1) {
