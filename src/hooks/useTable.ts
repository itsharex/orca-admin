...
export const useTable = (
  ...
  const getTableList = async () => {
    ...
    try {
      ...
      let { data, total } = await api({ ...state.searchInitParam, ...state.totalParam });
      let newData = { data: data, total: total, ...state.searchInitParam, ...state.totalParam };
      dataCallBack && (data = dataCallBack(newData));
      state.tableData = isPageable ? data.list : data;
      // 解构后台返回的分页数据 (如果有分页更新分页信息)
      if (isPageable) {
        const { page, pageSize } = newData;
