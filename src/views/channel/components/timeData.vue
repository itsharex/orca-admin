<template>
  <div class="table-box p10">
    ...
    <!-- 时间纬度详情弹窗 -->
    <TimeDialog v-model:dialogVisible="dialogTableVisible" :channel-id="channelId" :date="formInline.date" />
  </div>
</template>
<script setup lang="ts" name="timeData">
import { ref, onMounted, reactive } from "vue";
import { useTransition } from "@vueuse/core";
import dayjs from "dayjs";
import { numberformat } from "@/utils/index";
import { getTimeDimensionData, getTimeDimensionBoard } from "@/api/modules/channelData";
...
const getDimensionList = async () => {
  const res = await getTimeDimensionData(formInline);
  tableData.value = res.data;
};
// 获取数据看板
const getdimensionStatistics = async () => {
  const res = await getTimeDimensionBoard(formInline.date);
