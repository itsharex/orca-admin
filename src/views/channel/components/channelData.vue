<template>
  <div class="table-box p10">
    <el-row :gutter="20">
      <el-col :span="24">
        <el-select v-model="formInline.channelId" placeholder="请选择渠道" @change="handleChannelIdChange" class="w250">
          <el-option v-for="item in channelData" :key="item.channelId" :label="item.channelName" :value="item.channelId" />
        </el-select>
      </el-col>
    </el-row>
    ...
    <!-- 渠道纬度详情弹窗 -->
    <channelDialog />
    <!-- v-model:dialogVisible="dialogTableVisible" :channel-id="channelId" -->
  </div>
</template>
<script setup lang="ts" name="channelData">
import { ref, onMounted } from "vue";
import { useTransition } from "@vueuse/core";
import { getChannelList } from "@/api/modules/channel";
import { numberformat } from "@/utils/index";
import { getChannelDimensionData, getChannelDimensionBoard } from "@/api/modules/channelData";
...
const getDimensionList = async () => {
  const res = await getChannelDimensionData(formInline);
  tableData.value = res.data as never[];
};
// 获取数据看板
const getdimensionStatistics = async () => {
  const res = await getChannelDimensionBoard(formInline.channelId);
...
onMounted(() => {
  getChannelList({ page: 1, pageSize: 999 } as any).then(res => {
    channelData.value = res.data as { channelId: number; channelName: string }[];
    formInline.channelId = (res.data as any)[0].channelId;
    getdimensionStatistics();
    getDimensionList();
  });
});
